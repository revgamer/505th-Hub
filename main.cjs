...
function sendUpdateStatus(data){mainWindow?.webContents?.send('update:status',data);}
autoUpdater.autoDownload=true;autoUpdater.autoInstallOnAppQuit=false;
autoUpdater.on('checking-for-update',()=>{sendUpdateStatus({state:'checking'});});
autoUpdater.on('error',err=>{sendUpdateStatus({state:'error'});if(manualUpdateCheck){manualUpdateCheck=false;dialog.showMessageBox(mainWindow,{type:'error',title:'505th Hub',message:'Could not check for updates.',detail:String(err?.message||err)});}else console.error('[505th Hub] update check failed:',err);});
autoUpdater.on('update-not-available',()=>{sendUpdateStatus({state:'up-to-date',version:app.getVersion()});if(manualUpdateCheck){manualUpdateCheck=false;dialog.showMessageBox(mainWindow,{type:'info',title:'505th Hub',message:`You're up to date.`,detail:`Running version ${app.getVersion()}.`});}});
autoUpdater.on('update-available',info=>{manualUpdateCheck=false;sendUpdateStatus({state:'downloading',version:info.version});if(tray)tray.setToolTip(`505th Hub — downloading update ${info.version}…`);});
autoUpdater.on('update-downloaded',info=>{sendUpdateStatus({state:'ready',version:info.version});if(tray)tray.setToolTip('505th Hub — update ready to install');const res=dialog.showMessageBoxSync(mainWindow,{type:'info',title:'505th Hub update ready',buttons:['Restart now','Later'],defaultId:0,cancelId:1,message:`505th Hub ${info.version} has been downloaded.`,detail:'Restart now to finish installing it, or keep working and it will install next time you quit.'});if(res===0){quitting=true;autoUpdater.quitAndInstall();}});
...
app.whenReady().then(()=>{
 ...
 ipcMain.handle('app:version',e=>e.sender===mainWindow?.webContents?app.getVersion():'');
 ipcMain.handle('settings:get-login-item',e=>e.sender===mainWindow?.webContents?app.getLoginItemSettings().openAtLogin:false);
 ipcMain.handle('settings:set-login-item',(e,val)=>{if(e.sender!==mainWindow?.webContents)return false;app.setLoginItemSettings({openAtLogin:!!val});tray?.setContextMenu(trayMenu());return app.getLoginItemSettings().openAtLogin;});
 ipcMain.on('shell:open-external',(e,url)=>{if(e.sender!==mainWindow?.webContents)return;if(typeof url==='string'&&/^https:\/\//.test(url))shell.openExternal(url);});
 ipcMain.on('update:restart',e=>{if(e.sender!==mainWindow?.webContents)return;quitting=true;autoUpdater.quitAndInstall();});
 ipcMain.on('update:check-manual',e=>{if(e.sender!==mainWindow?.webContents)return;checkForUpdates(true);});
